...
// Client is a minimal JSON-RPC client over net/http.
type Client struct {
	endpoint string
	http     *http.Client
	maxRetry int
...
// call performs a JSON-RPC call with exponential backoff retry.
...
// blockHeader is used for full=false calls, where the RPC returns transactions
// as an array of hash strings. It deliberately omits Transactions so unmarshal
// does not try to decode hash strings into Transaction structs.
...
func (c *Client) GetBlock(ctx context.Context, n int64) (*Block, error) {
	res, err := c.call(ctx, "eth_getBlockByNumber", intToHex(n), true)
	if err != nil {
		return nil, err
	}
	var b Block
